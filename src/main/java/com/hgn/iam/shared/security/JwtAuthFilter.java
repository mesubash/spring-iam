package com.hgn.iam.shared.security;
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if (issuer != null && !issuer.isBlank() && !issuer.equals(claims.getIssuer())) {
                log.debug("Service JWT issuer mismatch — passing to next filter");
                filterChain.doFilter(request, response);
                return;
            }
            if (audience != null && !audience.isBlank()) {
                ...
                if (!audienceMatches(aud)) {
                    log.debug("Service JWT audience mismatch — passing to next filter");
                    filterChain.doFilter(request, response);
                    return;
                }
            }
            ...
            // Token not signed with service-to-service secret — pass through to AuthN JWT filter
            log.debug("Service JWT validation failed, passing to next filter: {}", e.getMessage());
        }
